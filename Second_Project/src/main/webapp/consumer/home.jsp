...
<div class="album py-5 bg-body-tertiary">
	...
	<% for(ProductBean pb : plists){ 
		if(pb.getStock()==0){%>
			<div class="col">
	          <div class="card shadow-sm">
	            <a href = "main_user_prod_view.jsp?pnum=<%=pb.getPnum()%>">
	            ...
	            <div class="card-body">
	              ...
	              <div class="d-flex justify-content-between align-items-center">
	                <div class="btn-group">
	                  <button type="button" class="btn btn-sm btn-outline-secondary" disabled onclick = "location.href='main_user_order.jsp?pnum=<%=pb.getPnum()%>'"><font style="vertical-align: inherit;">구매</font></button>
	                  <button type="button" class="btn btn-sm btn-outline-secondary" disabled onclick = "location.href='main_user_cartAdd.jsp?pnum=<%=pb.getPnum()%>'"><font style="vertical-align: inherit;">장바구니</font></button>
	                </div>
	              </div>
	            </div>
	          </div>
        ...
        <div class="col">
          <div class="card shadow-sm">
            <a href = "<%=cPath%>/consumer/prodView.jsp?pnum=<%=pb.getPnum()%>"><img class="bd-placeholder-img card-img-top" src = "<%=cPath%>/pimages/<%=pb.getPimage()%>" width="100%" height="225"></a>
            <div class="card-body">
              <p class="card-text"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;"> <%= pb.getPname() %><br> </font>
              <font class="text-body-secondary" style="vertical-align: inherit;" size = "2">
              <% if(pb.getWeight()>=1000){ %>
              ...
              <div class="d-flex justify-content-between align-items-center">
                <div class="btn-group">
                  <button type="button" class="btn btn-sm btn-outline-secondary" onclick = "location.href='calculate.jsp?pnum=<%=pb.getPnum()%>&pname=<%=pb.getPname()%>&amount=<%=pb.getPrice()%>'"><font style="vertical-align: inherit;">구매</font></button>
                  <button type="button" class="btn btn-sm btn-outline-secondary" onclick = "location.href='cartAdd.jsp?pnum=<%=pb.getPnum()%>'"><font style="vertical-align: inherit;">장바구니</font></button>
                </div>
              </div>
            </div>
          </div>
